...
# print(args)
founderPop = runMacs(nInd = 100, nChr = 1, segSites = 1000, inbred=TRUE)
...
write.table2(pedigree, "genotypes/pedigree_dh.txt")
write.table2(cbind(pedigree[,1], genotypes), "genotypes/trueGenotypes_dh.txt")
write.table2(cbind(rep(pedigree[,1], each = 2), haplotypes), "genotypes/trueHaplotypes_dh.txt")
...
write.table2(cbind(pedigree[,1], maskedGenotypes), "genotypes/maskedGenotypes_dh.txt")
write.table2(cbind(rep(pedigree[,1], each = 2), maskedHaplotypes), "genotypes/maskedHaplotypes_dh.txt")
